class Solution(object):
    ...
    def quick_sort_stack(self, nums):
        if len(nums) < 2:
            return nums
        ...
        while stack:
            [l, r] = stack.pop()
            idx = self.helper_quick_sort(nums, l, r)
            if l < (idx - 1):
                stack.append([l, idx-1])
            if (idx + 1) < r:
                stack.append([idx+1, r])

    def helper_quick_sort(self, nums, l, r):
        if l >= r:
            return None
